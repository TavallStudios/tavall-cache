 ...
 */
public final class CacheMap implements CacheStatsProvider {

    public static final String CACHE_NAME = "legacy-cache-map";
    ...
    /**
     * Builds a snapshot of current wrapper occupancy across the legacy cache map.
     *
     * @return immutable snapshot metrics
     */
    public CacheMetaData snapshotStats() {
        int totalEntries = 0;
        int validEntries = 0;
        int expiredEntries = 0;
        long now = System.currentTimeMillis();

        for (CopyOnWriteArrayList<ICacheValue<?>> bucket : buckets.values()) {
            for (ICacheValue<?> value : bucket) {
                totalEntries++;
        ...
        return new CacheMetaData(totalEntries, validEntries, expiredEntries);
    }
     ...
     */
    private List<List<ICacheValue<?>>> findBy(java.util.function.Predicate<Map.Entry<ICacheKey<?>, CopyOnWriteArrayList<ICacheValue<?>>>> predicate) {
